...
public:
	...
	typedef ptrdiff_t difference_type; //마지막 요소 - 가리키고 있는 포인터
...
private:
	...
	//3. 유틸리티 함수들(push_back 지원 함수)
	
	void grow();
	void grow_once(); //11-9.cpp을 위해 추가
...
void Vec<T>::create(size_type n, const T& val) {
	data = alloc.allocate(n); //#include <alloc>
	limit = avail = data + n;
...
void Vec<T>::grow() {
	...
	// old space(예전 공간)를 소멸
	uncreate();

	//모든 포인터를 새로운 공간에서 가리키기
	data = new_data;
	avail = new_avail;
	limit = data + new_size;
}
